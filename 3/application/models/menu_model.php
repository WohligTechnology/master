...
// GROUP BY `option1id`,`option2id`) as `table2` ON `table1`.`id1`=`table2`.`id1` AND `table1`.`id2`=`table2`.`id2`
       $query=$this->db->query("SELECT `user1`,`user2`,`question1`,`option1`,`question2`,`option2`,`count` as `countuser`,`optiontable1`.`text` as `option2`,`optiontable2`.`text` as `option1` FROM (SELECT `user1`,`user2`,`question1`,`option1`,`question2`,`option2`,count(`user1`) as `count` FROM (SELECT `user` as `user2`, `option` as `option2`,`question` as `question2` FROM `hq_useranswer` WHERE `question` = '$question1') as `tab1` INNER JOIN  (SELECT `user` as `user1`,`option` as `option1`,`question` as `question1` FROM `hq_useranswer` WHERE `question` = '$question2') as `tab2` ON `user1` = `user2` GROUP BY `option1`,`option2`) as `table1` INNER JOIN `hq_options` as `optiontable1` ON `table1`.`option1`= `optiontable1`.`id`  

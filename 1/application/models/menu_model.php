...
public function uploadImage(){
    ...
    public function enablemenu($package)
    {
        ...
        if($package==1){
             ...
             $query=$this->db->query("UPDATE `menuaccess` SET `access`=0 WHERE `menu` IN (17,15,18)");
             $query1=$this->db->query("UPDATE `user` SET `isfirst`=1 WHERE `accesslevel` = 1");
        }
        else if($package==2){
             $query=$this->db->query("UPDATE `menuaccess` SET `access`=1 WHERE `menu` IN (1,2,3,4,5,6,7,8,9,12,14,15,17)");
             $query1=$this->db->query("UPDATE `user` SET `isfirst`=1 WHERE `accesslevel` = 1");
        }
        else if($package==3){
             $query=$this->db->query("UPDATE `menuaccess` SET `access`=1 WHERE `menu` IN (1,2,3,4,5,6,7,8,9,12,14,15,17)");
